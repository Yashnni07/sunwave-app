...
function chaincodeInvokeInit() {
  ...
  echo "Attempting to invoke chaincode with function: ${CC_INIT_FCN} on channel: ${CHANNEL_NAME} with MAX_RETRY: ${MAX_RETRY} and DELAY: ${DELAY}"
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    ...
    let rc=$res
    if [ $rc -ne 0 ]; then
      echo "Attempt ${COUNTER} failed with return code ${rc}. Retrying..."
      echo "Log output from failed attempt:"
      cat log.txt
    fi
    COUNTER=$(expr $COUNTER + 1)
  done
  ...
  if [ $rc -eq 0 ]; then
    echo "Chaincode invoke init successful after ${COUNTER} attempts."
  else
    echo "Chaincode invoke init failed after ${COUNTER} attempts."
  fi
